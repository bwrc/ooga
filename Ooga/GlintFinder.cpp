		...
		bool bUpdateGlintModel )*/
{
	...
	//		int a = eyeImage_diff.size().width;
	...
	*/
	...
	/*	int k = 0;
		//!!this presupposes that the assigned arrays are initialized with zeros, and doesn't work? Check the logic with Miika?
		for (int j = 0; j<N_glint_candidates; j++) {
		for (int n = 0; n > N_leds; ++n){
		glints_x[j*N_leds + n][n] = glint_candidates[0][k];
		glints_y[j*N_leds + n][n] = glint_candidates[1][k];
		lhoods[j*N_leds + n][n] = loglkh_at_candidates[k];
		maps[j*N_leds + n][n] = loglkh_at_candidates[k];
		}
		++k;
		}
		*/
	...
	for (int n = 0; n < N_glint_candidates; n++) {
		for (int j = 0; j < N_leds; j++) {
			...
			for (int k = 1; k < N_leds; k++) { //operate on the same clone from this point on
				...
				//!! HÄTÄFIKSI - Tämä aiheuttaa systemaattisen kaatumisen alussa?
